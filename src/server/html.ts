...
export function getPage(
    ...
    const days = history
        ...
        : state.dates
    return `<!doctype html><html lang="cs"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${history ? 'Historie · ' : ''}PRE · Časy nízkého tarifu</title><link rel="stylesheet" href="/styles.css"><script src="/app.js" defer></script></head>
    <body data-refreshing="${state.refreshing}"><a class="skip" href="#main">Přejít na obsah</a><div class="shell"><header><a class="brand" href="/"><span class="brand-icon" aria-hidden="true">ϟ</span><span>Domácí energie<small>ROZVRH PRE</small></span></a><nav aria-label="Hlavní navigace"><a href="/" ${!history ? 'aria-current="page"' : ''}>Přehled</a><a href="/history" ${history ? 'aria-current="page"' : ''}>Historie</a></nav></header>
    <main id="main"><div class="intro"><div><p class="eyebrow">MÉNĚ NÁKLADŮ, LEPŠÍ PŘEHLED</p><h1>${history ? 'Historie spínání.' : 'Kdy běží levnější proud?'}</h1><p class="lead">${history ? 'Uložené rozvrhy nízkého tarifu, od nejnovějšího dne.' : 'Časy nízkého tarifu na dnes a zítra. Vše na jednom místě.'}</p></div><form action="/refresh" method="post"><button type="submit">↻ &nbsp; Aktualizovat</button></form></div>
...
:root{font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#173a36;background:#f4f6f2;font-synthesis:none;line-height:1.5}*{box-sizing:border-box}body{margin:0}a{color:inherit}button,a{-webkit-tap-highlight-color:transparent}a:focus-visible,button:focus-visible{outline:3px solid #b96624;outline-offset:5px}.shell{max-width:1140px;padding:0 38px;margin:auto}header{display:flex;justify-content:space-between;align-items:center;gap:20px;padding:32px 0;border-bottom:1px solid #dce2d9}.brand{display:flex;gap:12px;align-items:center;text-decoration:none;font-size:17px;font-weight:650;white-space:nowrap}.brand-icon{display:grid;place-items:center;width:42px;height:42px;border-radius:13px;background:#174f45;color:#d0eb99;font-size:31px}.brand small{display:block;font-size:9px;letter-spacing:2px;color:#61766b;margin-top:2px}nav{display:flex;gap:6px}nav a{text-decoration:none;font-size:14px;padding:10px 17px;border-radius:24px}nav a[aria-current]{background:#e1e9dc;font-weight:600}.intro{display:flex;justify-content:space-between;align-items:center;gap:24px;margin:52px 0 28px}.eyebrow{font-size:10px;letter-spacing:1.7px;font-weight:700;color:#567269;margin:0 0 12px}h1{font-size:clamp(29px,4vw,43px);line-height:1.17;font-weight:600;letter-spacing:-1.7px;margin:0 0 13px}.lead{color:#65766c;font-size:15px;margin:0}button{background:#fff;border:1px solid #d6dfd2;border-radius:9px;padding:12px 19px;color:#234e42;font-family:inherit;font-weight:600;font-size:13px;cursor:pointer;white-space:nowrap;min-height:44px}button:hover{background:#e9efdf}.status-banner{display:flex;gap:14px;align-items:center;border-radius:13px;padding:22px 25px;background:#e4eade;margin:0 0 28px}.status-banner.is-low{background:#214f44;color:#f4fbe9}.status-banner strong{font-size:16px;font-weight:600}.status-banner p{font-size:13px;margin:3px 0 0;opacity:.8}.status-dot{width:10px;height:10px;flex-shrink:0;border-radius:50%;background:#63766a}.is-low .status-dot{background:#c6e8a1;box-shadow:0 0 0 5px #ffffff12}.plan-label{margin-left:auto;font-size:9px;letter-spacing:1.3px;white-space:nowrap;opacity:.7}.cards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:22px}.day-card{padding:27px;border-radius:17px;border:1px solid #dee5da;background:#fff;min-width:0}.card-heading{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:28px}.card-heading .eyebrow{margin-bottom:6px}h2{font-weight:600;font-size:21px;letter-spacing:-.5px;margin:0}h2:first-letter{text-transform:uppercase}.hours{text-align:right;white-space:nowrap;font-size:23px;font-weight:600}.hours small{font-size:10px;display:block;font-weight:400;color:#738175}.timeline{height:18px;background:#edf0e8;position:relative;border-radius:5px;overflow:hidden}.timeline span{position:absolute;height:100%;background:#3a7860}.axis{display:flex;justify-content:space-between;color:#778479;font-size:10px;margin-top:7px}.intervals{list-style:none;margin:23px 0 18px;padding:0}.intervals li{display:flex;gap:15px;align-items:center;border-top:1px solid #eef1ea;padding:16px 0;font-size:15px}.intervals strong{font-size:20px;letter-spacing:-.5px;font-weight:500;white-space:nowrap;font-variant-numeric:tabular-nums}.interval-number{font-size:10px;color:#75897b}.dash{color:#879e8c;padding:0 5px}.interval-duration{margin-left:auto;font-size:12px;color:#65776b;white-space:nowrap}.updated{font-size:10px;color:#6a7c6f;margin:0}.stale{color:#926323}.legend{display:flex;flex-wrap:wrap;gap:22px;font-size:11px;color:#65776b;margin:24px 0 48px}.legend span{display:flex;align-items:center;gap:7px}.legend span:last-child{margin-left:auto}.legend i{display:inline-block;width:9px;height:9px;border-radius:2px;background:#3a7860}.legend .high{background:#e1e6d9}.notice{background:#fff2d9;border:1px solid #e9d6b1;padding:17px 20px;border-radius:12px;font-size:13px;margin:0 0 23px;color:#705121}.notice p{margin:5px 0 0}.empty{font-size:15px;color:#708175;padding:24px 0}.empty p{font-size:12px}footer{display:flex;justify-content:space-between;gap:20px;border-top:1px solid #dce2d9;padding:23px 0 34px;color:#6e7d70;font-size:11px}footer p{margin:0;max-width:630px}footer a{white-space:nowrap;text-underline-offset:3px}.skip{position:absolute;left:-9999px}.skip:focus{left:20px;top:10px;background:white;padding:10px;z-index:5}
